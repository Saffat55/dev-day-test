...
All settings (.json files) are now committed and stored in the project git repo. This means we will need to pull down and sync settings from the store before pushing any changes.

We have a script that automates this. First, rename `sync.yml.example` to `sync.yml` and update with your store credentials. (Should have theme and file read/write permissions)

Before syncing settings, you should commit any local changes and start from a clean git history.

Run `yarn sync` and select which theme to pull settings from.

You can also pull settings from your Development theme when developing locally to commit any settings changes you need to the repo, as changes made in your development theme from the customizer are not automatically pulled down and will be lost the next time you build or update a json file.
...
`Before beginning, it's recommended to first sync the settings (see above)`
