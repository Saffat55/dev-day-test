...
7. Upload the ZIP to your store under `Online Store > Themes`

8. Create a `config.yml` file in your projects root folder and add all of your store details to it.

    Example `config.yml` file structure.
    
    **NEW**: For improved performance with local development using Hot Module Reloading, it's recommended to ignore `.js` files within the development ThemeKit environment (see below example).
    Since we're using HMR, watching/deploying .js files is unneccessary and seems to be eating into performance. Note, running `yarn deploy` on the development environment will no longer deploy production .js files. You should instead use another ThemeKit environemnt for deploying such as `staging`.
...
### Commands
`yarn start` - This will start a process that will watch your `/src/` directory for changes and upload them to Shopify.

`yarn build` - This will build a `dist` folder containing all of your compiled code.

`yarn deploy` - This will deploy the theme to the specified environment. By default this will deploy to the development environment in the `config.yml` file. To specify an alternate environment add a flag to the command.
