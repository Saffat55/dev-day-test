...
const fileKey = 'IHs7tNLSZuYEZldeMxOwRf'
...
const getNodes = async () => {
  ...
  const figmaRes = await axios.get(`https://api.figma.com/v1/files/${fileKey}/styles`, {
  ...
  return await axios.get(`https://api.figma.com/v1/files/${fileKey}/nodes?ids=${nodesID}`, {
    headers: { 'X-Figma-Token': FIGMA_TOKEN }
  })
...
(async () => {
  try {
    cancelKeys()
    const figmaFileKey = await input('\nInput Figma File Key')
    const figmaNodes = await getNodes()
