function addToCart () {
  ...
  productSubmitButtons.forEach((productSubmitButton) => {
    productSubmitButton.addEventListener('click', () => {
      const productForm = productSubmitButton.closest('[data-product-form]')
      const quantityInput = productForm.querySelector('[data-quantity-input]')
      const lineItemVariantId = parseInt(productForm.querySelector('[data-product-select]').value, 10)
      const lineItemQuantity = quantityInput ? parseInt(quantityInput.value, 10) : 1
...
function quantitySelector () {
  document.addEventListener('change', (event) => {
    ...
    const parent = event.target.closest('[data-line-item]')
    const lineItemKey = parent.getAttribute('data-line-item-key')
